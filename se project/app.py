...
def index():
    ...
    if request.method == "POST":
        ...
        if file:
            ...
            text_resul  = extract_text_from_image(poster_path)
            text_result=f"Text extracted from poster: {text_resul}"

            qr_code_result = validate_qr_code(poster_path)

            return f"{average_rgb_result}<br>{indentation_result}<br>{size_dimension_result}<br>{clarity_result}<br>{clutter_result}<br>{text_result}<br>QR Code Data: {qr_code_result}"
